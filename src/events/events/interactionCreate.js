...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (timestamps.has(interaction.user.id)) {
      ...
      if (now < expirationTime) await interaction.reply({
        embeds: [
            errorEmbed
              .setTitle("⏱️ Cooldown")
              .setDescription(`**${interaction.user.username}**! Slow down and try the command again <t:${ Math.round(expirationTime / 1000) }:R>`)
        ],
        ephemeral: true
      })
