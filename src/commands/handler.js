    ...
    for (const file of commandFiles) {
      ...
      if ("name" in command && "description" in command) {
        ...
        const subs = command.options.filter(
          (k) => k.type == ApplicationCommandOptionType.Subcommand
        );
        if (subs.length) {
          for (const sub of subs) {
            const subcommand = require(
              path.join(commandsPath, "sub", `${sub.name}.${command.name}.js`)
            );
        ...
        client.logger.warning(
          `The command at ${filePath} is missing a required property.`
        );
      }
    }
  }
